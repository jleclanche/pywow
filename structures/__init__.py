...
class DBStructure(list):
	...
	def __init__(self, build=0):
		...
		def _schedule_change(col):
			...
			_scheduled.append((col, i))
			return "%s_%i" % (col.name, i)
		...
		for col in self.columns:
			if isinstance(col, DynamicFields):
				names.append(col[0].name)
				names.count(col[0].name) < 2 or _schedule_change(col[0])
				self.append(col[0])
				for _col in col[1:]:
					for __col in _col:
						names.append(__col.name)
						names.count(__col.name) < 2 or _schedule_change(__col)
						self.append(__col)
			
			elif isinstance(col, LocalizedFields):
				for _col in col:
					names.append(_col.name)
					names.count(_col.name) < 2 or _schedule_change(_col)
					self.append(_col)
		...
		for col in _scheduled:
			col[0].name = "%s_%i" % (col[0].name, col[1])
		
		self.column_names = [k.name for k in self]
...
class _Generated(DBStructure):
	...
	def __init__(self, structure_string, *pargs, **kwargs):
		columns = []
		fields = {"i": IntegerField, "s": StringField}
		self.base = Skeleton(*[fields[s]() for s in structure_string])
...
class ItemCache(DBStructure):
	...
	base = Skeleton(
		...
		StringField("name"),
		StringField("name2", deprecated=True),
		StringField("name3", deprecated=True),
		StringField("name4", deprecated=True),
...
#		), 10]),
		...
		IntegerField("armor"),
		IntegerField("holyresist", deprecated=True),
		...
		IntegerField("socket1info", deprecated=True),
		BitMaskField("socket2"),
		IntegerField("socket2info", deprecated=True),
		BitMaskField("socket3"),
		IntegerField("socket3info", deprecated=True),
...
class ItemRandomProperties(DBStructure):
	...
	base = Skeleton(
		IDField(),
		StringField("name", deprecated=True),
		IntegerField(),
		IntegerField(),
		IntegerField(),
		IntegerField(unused=True),
