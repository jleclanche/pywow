...
errors = {
	...
	-5: (IOError, "write"),
	-6: (MemoryError,),
	-7: (Error, "file does not exist, is not an mpq or is corrupted"),
...
class MPQArchive(object):
	def __init__(self, source, ctypes=ctypes, File=File, libmpq=libmpq):
		...
		libmpq.libmpq__archive_open(ctypes.byref(self._mpq), self.filename, ctypes.c_uint64(offset))
		self._opened = True
	...
	def __repr__(self):
		return "MPQArchive(%r)" % (self._source)
	...
	def __contains__(self, item, ctypes=ctypes, libmpq=libmpq):
		if isinstance(item, str):
			...
			try:
				libmpq.libmpq__file_number(self._mpq, ctypes.c_char_p(item), ctypes.byref(data))
			except IndexError:
				return False
			return True
...
# Remove clutter - everything except Error and MPQArchive.
